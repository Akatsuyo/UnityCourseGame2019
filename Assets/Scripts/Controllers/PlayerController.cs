...
public class PlayerController : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D other) {
        ...
        if (isEnemy) {
            ...
            knockbacked = true;
            audio.PlayOneShot(hurtAudio, 0.8f);
        }
    }

    void OnPickupCoin(object sender, EventArgs e) {
        audio.PlayOneShot(coinPickupAudio, 0.6f);
    }
    ...
    {
        if (!health.HasHealth())
            Global.GameController.ResetCurrentLevel();
    }
